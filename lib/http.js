...
module.exports = class eWeLinkHTTP {
  ...
  async getHost() {
    ...
    try {
      ...
      if (this.debugReqRes) {
        this.log("Sending HTTP getHost() request. This text is yellow for clarity.\n%s", JSON.stringify(params, null, 2));
      } else if (this.debug) {
  ...
  async login() {
    ...
    if (this.debugReqRes) {
      ...
      this.log("Sending HTTP login() request. This text is yellow for clarity.\n%s", msg);
    } else if (this.debug) {
    ...
    const body = res.data;
    if (util.hasProperty(body, "error") && body.error === 10004 && util.hasProperty(body, "data") && util.hasProperty(body.data, "region")) {
      ...
      if (body.data.at) {
        ...
        if (body.error === 500) {
          if (this.debug) this.log("An eWeLink error [500] occured. Retrying in 30 seconds.");
          await util.sleep(30000);
          return await this.login();
        } else {
  ...
  async getDevices() {
    try {
      ...
      const body = res.data;
      if (!util.hasProperty(body, "data") || !util.hasProperty(body, "error") || (util.hasProperty(body, "error") && body.error !== 0)) {
      ...
      if (body.data.thingList && body.data.thingList.length > 0) {
        body.data.thingList.forEach((d) => {
          if (util.hasProperty(d, "itemData") && util.hasProperty(d.itemData, "extra") && util.hasProperty(d.itemData.extra, "uiid") && !this.hideDevFromHB.includes(d.itemData.deviceid)) {
