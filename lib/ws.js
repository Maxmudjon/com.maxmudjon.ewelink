  ...
  login() {
    ...
    this.wsp.onOpen.addListener(async () => {
      ...
      if (this.debugReqRes) {
        ...
        this.log("Sending WS login request. This text is yellow for clarity.\n%s", msg);
      } else if (this.debug) {
        ...
        switch (device.action) {
          ...
          case "sysmsg":
            ...
            for (const param in device.params) {
              if (util.hasProperty(device.params, param)) {
    ...
    this.wsp.onClose.addListener(async (e) => {
      ...
      if (e === 1005) return;
      if (this.debug) {
        this.log("Web socket closed - [%s].", e);
    ...
    if (this.wsp && this.wsIsOpen) {
      try {
        if (this.debugReqRes) {
          ...
          this.log("WS message sent. This text is yellow for clarity.\n%s", msg);
        } else if (this.debug) {
